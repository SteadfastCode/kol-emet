...
| POST | `/auth/webauthn/register/begin` · `/complete` | Add a passkey (`requireAuth`): the prompt after signup, and Settings → Passkeys. `/complete` answers 409 when the credential id is already registered — to this account or any other, with the same message either way (WebAuthn §7.1) |
...
| POST | `/auth/webauthn/login/begin` · `/complete` | Passwordless login via passkey. With an email or without one (discoverable credentials). Records the passkey's `lastUsedAt` and sync status. `/complete` is throttled by client address (no email in its body), sharing that counter with `/auth/login`: 429 with a `Retry-After` |
