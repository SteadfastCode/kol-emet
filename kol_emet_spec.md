...
- **A credential id may be registered once, to one account (decided 2026-09-17, KOL-036)**: `POST /auth/webauthn/register/complete` stored whatever verified, so the same credential id could sit on two accounts. WebAuthn §7.1 says not to, and the reason is `lib/passkeyIds.js`'s own lookup: sign-in finds the account by `User.findOne(credentialIdQuery(id))`, which with two holders returns whichever mongod hands back first. The route now asks `User.exists(credentialIdQuery(...))` after verification and before the push, and answers `409 {"error": "This passkey is already registered"}` without saving.
  ...
  - **Both stored forms block.** The query is the same one sign-in uses, so an id held in the KOL-024 legacy double-encoded form refuses its correct form too. Otherwise the fix would have a hole in exactly the data it exists to protect.
